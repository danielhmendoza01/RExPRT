...
run_bedtools_operation() {
    ...
    ./bedtools.static.binary intersect -a $INPUT_FILE -b $annotation_file -c > $OUTPUT_DIR/intersection_$operation
...
if command -v parallel >/dev/null 2>&1; then
    ...
    parallel --no-notice -j 4 ::: \
        "run_bedtools_operation TAD $OUTPUT_DIR/tad_annotated.txt annotation_files/TADboundaries_CpGcount.bed TAD" \
        "run_bedtools_operation eSTR $OUTPUT_DIR/estr_annotated.txt annotation_files/eSTR_loci_hg19.sorted.bed eSTR" \
        "run_bedtools_operation opReg $OUTPUT_DIR/opreg_annotated.txt annotation_files/openRegulatoryRegions_hg19.sorted.bed opReg" \
        "run_bedtools_operation promoter $OUTPUT_DIR/promoter_annotated.txt annotation_files/promoters.sorted.bed promoter"
    ...
    ./bedtools.static.binary intersect -a final_annotated_parallel.txt -b annotation_files/eSTR_loci_hg19.sorted.bed -c > intersection
    cat header intersection > final_annotated_parallel.txt

    # Add opReg
    head -1 final_annotated_parallel.txt > header
    awk '{print $0 "\topReg"}' header > head && mv head header
    ./bedtools.static.binary intersect -a final_annotated_parallel.txt -b annotation_files/openRegulatoryRegions_hg19.sorted.bed -c > intersection
    cat header intersection > final_annotated_parallel.txt

    # Add promoter
    head -1 final_annotated_parallel.txt > header
    awk '{print $0 "\tpromoter"}' header > head && mv head header
    ./bedtools.static.binary intersect -a final_annotated_parallel.txt -b annotation_files/promoters.sorted.bed -c > intersection
    cat header intersection > final_annotated_parallel.txt
