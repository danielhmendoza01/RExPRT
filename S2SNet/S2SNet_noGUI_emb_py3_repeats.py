...
def process_single_sequence(args):
    ...
    sline, nSeq, k, g, np, nse, nwi, Mark = args
    ...
    try:
        (PDB, Chain, Seq) = sline.split("\t") # code, chain, seq
        if len(Seq)>5000:
            return None  # Skip large sequences
    except:
    ...
    if nwi==1: # only with Weight ON
        for iATS in range(1,np+1):
            ...
            nATS=ATS(d,iATS,Mw)
            result_line += "\t"+str(nATS)

    # Balaban
    nBal=Balaban(Mconect,d,nwi,Mw)
    result_line += "\t"+str(nBal)
    ...
    return result_line, statGr, PDB+Chain
...
def S2SGmain(emb, markov, power, sequences, groups,results,orig_dir):
    ...
    # Use configured CPU cores for multiprocessing
    try:
        # Try to get from environment variable first
        import os
        max_cores = os.environ.get('MAX_CPU_CORES')
        if max_cores:
            num_processes = int(max_cores)
        else:
    ...
    args_list = []
    for idx, sline in enumerate(slines):
        args_list.append((sline, idx+1, k, g, np, nse, nwi, Mark))

    # Process sequences in parallel
    print("-> Starting parallel processing...")
    with mp.Pool(processes=num_processes) as pool:
        results = pool.map(process_single_sequence, args_list)
    ...
    for idx, result in enumerate(results):
        ...
        result_line, statGr, seq_id = result
        SeqList.append(seq_id)
        statGrAll[processed_count] = statGr
        processed_count += 1

        # Write result line to file
        frFile.write(result_line + "\n")

        # Print progress
        if (idx + 1) % 10 == 0 or idx + 1 == len(results):
            print(f"Processed {idx + 1}/{len(results)} sequences")

    frFile.close() # close results file
    print(f"-> Completed processing {processed_count} sequences")
